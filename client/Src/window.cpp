...
inline void calculateControlsAndDraw(HWND hWnd)
{
	HDWP hDWP;
	RECT rc;
...
LRESULT CALLBACK MainWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_WINDOWPOSCHANGED:
		{
			if(SWP_NOSIZE & (reinterpret_cast<WINDOWPOS *>(lParam) -> flags))
			{
				// 如果大小没有变化则不去重绘控件
				break;
			}
...
void __stdcall windowThread()
{
	...
	// 加载链接comctl32.dll
	INITCOMMONCONTROLSEX initControlsEx{ 0 };
	initControlsEx.dwSize = sizeof(initControlsEx);
	initControlsEx.dwICC = ICC_STANDARD_CLASSES | ICC_WIN95_CLASSES;
	...
	// 创建在控件上使用的字体. We use MulDiv and GetDeviceCaps to convert
...
void SetUpWindow(HINSTANCE hInstance, std::unique_ptr<std::thread> &p_thread)
{
	g_hInst = hInstance;

	std::unique_lock<std::mutex> lock(g_mutex);
	p_thread = std::make_unique<std::thread>(windowThread);
